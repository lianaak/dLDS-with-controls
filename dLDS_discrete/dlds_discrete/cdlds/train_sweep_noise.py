...
def main(args):
    ...
    run = wandb.init(
        ...
        project=f"{str(args.num_subdyn)}_State_TRUE_Control_Noisy_Coeff_{project_name}_LSTM",
    ...
    with torch.no_grad():
        ...
        model.coeffs = torch.nn.Parameter(torch.tensor(one_hot_states, requires_grad=True, dtype=torch.float32) + torch.randn_like(model.coeffs) * args.sigma)
