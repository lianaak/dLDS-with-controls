...
class CdLDSDataGenerator:
    ...
    U_: np.ndarray = None
    z_: np.ndarray = None
    fix_point_change: bool = None

    def generate_data(self, T,  discrete_state_maxT=200, control_density=0.02, initial_conditions=None, sigma=0, eigenvalue_radius=0.99, fix_point_change=True):
        ...
        self.fix_point_change = fix_point_change

        # if A and B are not defined, create them
        if len(self.A) == 0 and self.B is None:
            self.create_dynamics(eigenvalue_radius=eigenvalue_radius)

        self.create_controls(
            T, discrete_state_maxT=discrete_state_maxT, control_density=control_density)
        ...
        x = self.multi_step_reconstruction(
            T=T, X0=initial_conditions, A=self.A, B=self.B,  U=self.U_, z=self.z_)
    ...
    def create_dynamics(self, eigenvalue_radius=0.99):
        """
        ...
        for k in range(self.K):
            ...
            A_k = (Q @ jordan @ np.linalg.inv(Q)).real*eigenvalue_radius
