...
def main(args):
    ...
    if args.wandb:
        run = wandb.init(
            ...
            project=f"PROTOTYPE_{str(args.num_subdyn)}_State_ALL_TRUE_With_Control_F_Random_Coeff_Random",
    ...
    fig = util.plotting(dummy_model.coeffs.detach().numpy()[
        :, :train_size].T, title='dummy coeffs', plot_states=args.plot_states, states=states[:train_size])
    # add axis labels
    ...
    fig = util.plotting(model.coeffs.detach().numpy()[
        :, :train_size].T, title='coefficients', plot_states=args.plot_states, states=states[:train_size])
    fig.update_xaxes(title_text='time')
    fig.update_yaxes(title_text='magnitude')
    ...
    fig = util.plotting(recon.detach().numpy()[:train_size, :], title='multi-step reconstruction',
                        stack_plots=False, plot_states=args.plot_states, states=states[:train_size])
    fig.update_xaxes(title_text='time')
    fig.update_yaxes(title_text='value')
    wandb.log({"multi-step reconstruction": fig})

    # result = model(time_series.float(), torch.arange(len(timeseries)-lookback))
    fig = util.plotting([timeseries[:train_size, :], recon_single.detach().numpy()[:train_size, :]
                         ], title='single-step + ground truth reconstruction', stack_plots=True, plot_states=True, states=states[:train_size])
    fig.update_xaxes(title_text='time')
    fig.update_yaxes(title_text='value')
