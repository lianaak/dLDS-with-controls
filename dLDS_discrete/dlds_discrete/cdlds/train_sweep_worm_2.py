...
class reshape_tensor(nn.Module):
    def __init__(self, hidden_size, lookback):
    ...
    def forward(self, x):
        ...
        # Reshape shape (batch, hidden)
        tensor = tensor.view(-1, self.lookback, self.hidden_size)
...
class AttentionLayer(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.hidden_size = hidden_size
        self.key = slim.linear.SpectralLinear(
                hidden_size , hidden_size , bias=False, sigma_min=0.0, sigma_max=1.0)
        # slim.linear.SpectralLinear(hidden_size, hidden_size, bias=False)
        self.query = slim.linear.SpectralLinear(
                hidden_size , hidden_size , bias=False, sigma_min=0.0, sigma_max=1.0)
        self.value = slim.linear.SpectralLinear(
                hidden_size , hidden_size , bias=False, sigma_min=0.0, sigma_max=1.0)
    ...
    def forward(self, x):
        ...
        # Attention mechanism
        attention = torch.matmul(query, key.transpose(1, 0)) / np.sqrt(self.hidden_size ) # (batch, seq_len, seq_len)
        ...
        # reshape to (batch x seq_len, hidden)
...
class DLDSModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, time_points, num_subdyn, lookback=50, control_size=1, sparsity_threshold=0.1):
        ...
        for _ in range(num_subdyn):
            ...
            self.F.append(nn.Sequential(f_i, extract_tensor(), attention, reshape_tensor(hidden_size*2, lookback), linear, nn.LayerNorm(output_size)))
...
def main(args):
    ...
    if args.generate_data:
        ...
        if args.time_points:
            ...
            controls = controls[:args.time_points]
    ...
    run = wandb.init(
        ...
        project=f"WORM_{str(args.num_subdyn)}_State_TRUE_Control_Coeff_{project_name}_LSTMwithAttention",
    ...
    fig = util.plotting([time_series[:, -1, :], result.detach().numpy()
                        [:, -1, :]], title='single-step reconstruction', stack_plots=True, plot_states=args.plot_states, states=states)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='worm_DS1_low.npy')
    ...
    parser.add_argument('--time_points', type=int, default=3329)
