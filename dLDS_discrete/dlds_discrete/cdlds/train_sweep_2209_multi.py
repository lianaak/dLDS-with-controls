...
class DLDSModel(nn.Module):
    ...
    def forward(self, X, idx):
        ...
        for t in idx:
            X_t = torch.stack([self.coeffs[i, t].view(-1, 1, 1)*f_i(X_t) + (self.B(
                self.effective_U[:, t].unsqueeze(0))).unsqueeze(1) for i, f_i in enumerate(self.F)]).sum(dim=0)  # removed .T, added unsqueeze(0)
...
def multi_step_reconstruction(model, n_time_points, X0):

    X_reconstructed = torch.zeros((model.input_size, n_time_points))
    # np.zeros((model.input_size, n_time_points))
    X_reconstructed[:, 0] = X0

    for t in range(1, n_time_points):
        X_reconstructed[:, t] = model(X_reconstructed[:, t-1], torch.arange(
            n_time_points))
...
def main(args):
    ...
    if args.wandb:
        run = wandb.init(
            ...
            project=f"PROTOTYPE_{str(args.num_subdyn)}_State_ALL_TRUE",
    ...
    for epoch in range(n_epochs):
        ...
        for (X_batch, y_batch), idx in loader:
            ...
            single_loss = loss_fn(y_pred, y_batch[-1])
            multi_loss = multi_step_loss(
                X_batch, y_batch, model, loss_fn, lookback, teacher_forcing_ratio)
            loss = smooth_reg_loss + \
                single_loss + args.control_sparsity_reg * \
                control_sparsity + coeff_loss + control_loss + args.loss_reg * multi_loss
    ...
    recon_single = torch.zeros_like(time_series)
    recon_single[:lookback] = time_series[:lookback]
    recon[:lookback] = time_series[:lookback]
    with torch.no_grad():
        for i in range(len(time_series)-lookback):
            y_pred = model(recon[i:i+lookback].float(),
                           torch.arange(i, i+lookback))
            y_pred_single = model(time_series[i:i+lookback].float(),
                                  torch.arange(i, i+lookback))
            recon[i+lookback] = y_pred  # [-1]
            recon_single[i+lookback] = y_pred_single

    fig = util.plotting(recon.detach().numpy()[
        :, -1, :], title='reconstruction', stack_plots=False, plot_states=args.plot_states, states=states[lookback-1:])
    wandb.log({"multi-step reconstruction": fig})

    # result = model(time_series.float(), torch.arange(len(timeseries)-lookback))

    # result = multi_step_reconstruction(
    #    model, len(timeseries), time_series[0, 0, :])
    fig = util.plotting([time_series[:, -1, :], recon_single.detach().numpy()
                         [:, -1, :]], title='result', stack_plots=True, plot_states=args.plot_states, states=states[lookback-1:])
