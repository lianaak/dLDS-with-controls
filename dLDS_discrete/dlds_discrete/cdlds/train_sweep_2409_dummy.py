...
def main(args):
    ...
    if args.wandb:
        run = wandb.init(
            ...
            project=f"DUMMY_{str(args.num_subdyn)}_State_ALL_TRUE_2409",
    ...
    with torch.no_grad():
        ...
        if args.generate_data:
            ...
            for f_i, A in zip(dummy_model.F, true_dynamics):
    ...
    with torch.no_grad():
        y_pred = dummy_model(X_train.float(), X_train_idx)
        train_rmse = np.sqrt(loss_fn(y_pred, y_train))
        y_pred = dummy_model(X_test.float(), X_test_idx)
        test_rmse = np.sqrt(loss_fn(y_pred, y_test))
    print("Epoch 0: train RMSE %.4f, test RMSE %.4f" %
    ...
    recon_single_dummy = torch.zeros_like(recon)
    recon_single_dummy[0] = ts[0]

    with torch.no_grad():
        for i in range(1, len(timeseries)):
            
            y_pred_single_dummy = dummy_model(ts[i-1].float().unsqueeze(0),
                                  i-1)
            recon_single_dummy[i] = y_pred_single_dummy[-1]

    
        # result = model(time_series.float(), torch.arange(len(timeseries)-lookback))
    fig = util.plotting([timeseries, recon_single_dummy.detach().numpy()
                         ], title='result', stack_plots=True, plot_states=args.plot_states, states=states)
    wandb.log({"dummy single-step + ground truth reconstruction": fig})

    # print(model.U.detach().numpy())

    # control plot
    fig = util.plotting(dummy_model.effective_U.detach().numpy()[
